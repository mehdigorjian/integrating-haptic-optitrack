...
Eigen::Vector3d curr_opti;
std::vector<Eigen::Vector3d> haptic_tip_positions;
std::vector<Eigen::Vector3d> opti_postitions;
...
void mainLoop(void) {
    ...
    while (1) {
        ...
        if (currentData.m_buttonState && !prevData.m_buttonState) {
            ...
            // adding haptic tip coordinate to the vector
            Eigen::Vector3d hpos(currentData.m_devicePosition[0],
                                 currentData.m_devicePosition[1],
                                 currentData.m_devicePosition[2]);
            haptic_tip_positions.push_back(hpos);

            // adding optitrack coordinate to the vector
            opti_postitions.push_back(curr_opti);
